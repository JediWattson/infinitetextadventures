...
const reducer = (state: { [key: string]: ModalInputRefType }, action: { type: string, payload: { ref: ModalInputRefType, key: string } }) => {
    switch (action.type) {
        case "set-ref":
...
const GameForm = ({ gameData, onClose }: { gameData?: { [key: string]: string }, onClose: () => void }) => {

    const [state, dispatch] = useReducer(reducer, {});

    const makeRef = (key: string) => (ref: ModalInputRefType) => {
        if (!ref || (state[key] && !(gameData && gameData[key] !== "" && ref.value === ''))) return;
    ...
    const handleSubmit = async () => {
        const reqObj = gameKeys.reduce((acc: { [key: string]: string }, game) => {
            ...
            if (gameData && game.value !== 'gameKey' && gameData[game.value] === ref.value)  return acc;
        ...
        <Modal>
            <>
                <div className={style.modalContainer}>
                    ...
                    {gameKeys.map((g) => 
                        <Input key={g.label} initValue={gameData && gameData[g.value]} label={g.label} inputRef={makeRef(g.value)} textArea={g.textArea} />
                    )}                
                </div>
